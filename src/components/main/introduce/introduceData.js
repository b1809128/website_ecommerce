export const introduceData = [
  {
    image: "./images/logo/logo.png",
    text: "Với hơn 10 năm kinh nghiệm trong lĩnh vực thiết kế giao diện web và công nghệ website trên mọi lĩnh vực công nghệ thông tin. Công ty Giải pháp Phần mềm TA sẽ mang đến cho bạn dịch vụ tốt nhất với các sản phẩm quốc tế.",
  },
  {
    image: "./images/logo/logo_apple.jpg",
    text: "Với sự hợp tác giữa Rabbit Technologies và Apple tại Việt Nam, chúng tôi luôn mang đến sự hài lòng về chất lượng sản phẩm cũng như dịch vụ chăm sóc khách hàng tốt nhất.",
  },
  {
    image: "./images/logo/logo_service.jpg",
    text: "Cam kết của chúng tôi về chất lượng dịch vụ khách hàng là một trong những điều quan trọng nhất của Rabbit Technologies",
  },
];
